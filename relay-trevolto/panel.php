...
//
//   Open in a browser:  https://hooks.trevolto.com/panel.php
...
function customer_msg(string $token): string {
    return "Your Trevolto licence key:\n\n{$token}\n\n"
...
function renewal_msg(string $token, int $exp): string {
    ...
    return "Hi — your Trevolto licence (key {$token}) "
...
if (!$authed) {
    ...
    $bad = ($provided !== null);
    head('Sign in — Trevolto Licences');
    echo '<div class="login-wrap"><div class="card login-card">';
    echo '<h1>Trevolto Licences</h1><p class="sub">Sign in with your admin key.</p>';
    if ($bad) {
...
if (($_GET['export'] ?? '') === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="trevolto_customers.csv"');
...
head('Trevolto — Licences');
?>
  <div class="topbar">
    <div>
      <h1>Trevolto — Customer Licences</h1>
      <p class="sub">
  ...
  <?php if ($result):
    ...
    if (strpos((string)$result['label'], '@') !== false) {
        $mailto = 'mailto:' . rawurlencode($result['label'])
                . '?subject=' . rawurlencode('Your Trevolto licence key')
