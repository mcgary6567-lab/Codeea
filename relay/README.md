# Prometheus Relay (shared-hosting, PHP + MySQL)
...
2. **Edit `config.php`** — fill `DB_*`, and set long random `SELLER_KEY` and
   `ADMIN_KEY`.
3. **Point the subdomain** `hooks.prometheusai.tech` at a folder and **upload
...
4. Test in a browser: `https://hooks.prometheusai.tech/` should print
   *"Prometheus relay is running."*
...
- Alert **Condition = "Any alert() function call"** (not "Prometheus BUY").
- **Webhook URL:** `https://hooks.prometheusai.tech/hook.php?key=YOUR_SELLER_KEY`

## Issue customer licences

```
Create:   https://hooks.prometheusai.tech/admin.php?key=ADMIN_KEY&action=create&label=John&days=30
List:     https://hooks.prometheusai.tech/admin.php?key=ADMIN_KEY&action=list
Revoke:   https://hooks.prometheusai.tech/admin.php?key=ADMIN_KEY&action=revoke&token=XXXX
Extend:   https://hooks.prometheusai.tech/admin.php?key=ADMIN_KEY&action=extend&token=XXXX&days=30
```
...
pre-filled to `https://hooks.prometheusai.tech/poll.php`). `days=0` (or omit) =
