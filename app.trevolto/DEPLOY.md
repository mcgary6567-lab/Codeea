...
1. **Pick a globally-unique app name.** Fly app names are shared across all Fly
   users, so `trevolto-web` may be taken. Edit `app.trevolto/fly.toml` line
   `app = "…"` to something unique (e.g. `trevolto-web-yourname`). Commit it.
...
3. **Add GitHub repo secrets:** your repo → **Settings → Secrets and variables →
   ...
   - `TREVOLTO_SECRET_KEY` = a long random string (e.g. from
     <https://generate-secret.vercel.app/32> or any password generator)
   - `TREVOLTO_ADMIN_EMAIL` = your email *(optional — makes you admin)*
4. **Run it:** repo → **Actions → Deploy Trevolto Web to Fly.io → Run
...
5. **Attach your subdomain** in the Fly dashboard (Certificates) or once, via
   `fly certs add app.trevolto.com`, then add the DNS records Fly shows.
