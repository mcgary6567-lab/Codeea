# Prometheus — cPanel / Shared Hosting Setup (no VPS, no SSH)

Any cPanel host with **"Setup Python App"** (Hostinger shared, GoDaddy cPanel,
Namecheap, etc.) can run Prometheus through Passenger.
...
Upload this app's zip and **Extract** it, e.g. `/home/USER/prometheus`.
...
| Application root | `prometheus` (the extracted folder) |
...
```
PROMETHEUS_SECRET_KEY   = <a long random string, 40+ chars>
PROMETHEUS_PUBLIC_URL   = https://yourdomain.com
PROMETHEUS_DATA_DIR     = /home/USER/prometheus/data
PROMETHEUS_ADMIN_EMAIL  = you@email.com
```
...
**Prefer real-time + easiest setup?** Use a VPS: `sudo bash install.sh` (see `Prometheus_Guide.pdf`).
