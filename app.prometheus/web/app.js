...
function setBotSigNote() {
  ...
  if (LAST_SIG) { note.style.color = LAST_SIG.buy ? "var(--green)" : "var(--red)"; note.innerHTML = `Last signal <b>${LAST_SIG.buy ? "▲ BUY" : "▼ SELL"} ${LAST_SIG.px}</b> · ${LAST_SIG.symbol}`; }
  else { note.style.color = ""; note.textContent = "No signal yet"; }
}
...
function lockStrategy(st) {
  ...
  // The upsell / request box shows only to VIP-eligible customers (paid + managed +
  // new) — or to anyone with a request already pending, so they still see its status.
  const showBox = !!(st.vip_eligible || st.custom_requested);
  if ($("strat-upgrade")) $("strat-upgrade").classList.toggle("hidden", !showBox);
...
function applySettings(s, email) {
  ...
  chk("s-weekend", s.weekend_pause); chk("s-weekend-local", !!s.weekend_tz);
  { const off = -new Date().getTimezoneOffset() / 60; const lbl = $("s-weekend-tz"); if (lbl) lbl.textContent = `(your device: UTC${off >= 0 ? "+" : ""}${off})`; }
...
const SPARAMS = { fast_ema: 9, slow_ema: 21, trend_ema: 100, use_trend_filter: 1, confirm: 2, min_body: 0.4, sl_ema_buffer_pct: 0.2, swing_lookback: 10, tp_r: 1.0, partial_pct: 0.5, whipsaw_max_crosses: 2, whipsaw_window: 5, whipsaw_suspend_hours: 12, post_sl_cooldown_bars: 0, avoid_daily_close: 1, aggressive_entries: 0 };
...
const BOOLP = new Set(["use_trend_filter", "avoid_daily_close", "aggressive_entries"]);
...
async function logoutAll() {
  ...
  try {
    ...
    if (d.token) { TOKEN = d.token; localStorage.setItem("prometheus_token", TOKEN); }
