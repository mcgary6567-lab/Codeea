# Pricing - Gold Scalpers Pro

*Last updated: 2026-06-27*

Gold Scalpers Pro is sold as a single one-time purchase. There is no subscription and no recurring fee.

## Pro Lifetime
- Price: $140 USD, one-time (regular price $220 - save $80)
- Billing: one-time payment, no subscription
- License: lifetime; bound to your MetaTrader 5 account number(s). Contact support to add or change accounts.
- Platform: MetaTrader 5 (MT5), any broker
- Market: XAUUSD (gold)
- Guarantee: 30-day money-back
- Payment methods: credit/debit card, PayPal, Google Pay, Apple Pay, and cryptocurrency
- Buy: https://goldscalpers.com/checkout

## What's included
- Full Gold Scalpers Pro for MetaTrader 5 and all future updates
- Slope Direction Line × 50 EMA crossover, confirmed by ZeroLag MACD and an optional confirmation candle
- Protective step-stop; quick scalping take-profit with optional Momentum Pyramiding (M1 timeframe)
- Risk controls: daily profit/loss limits, max-drawdown guard, max daily orders, spread/session/news filters
- Live on-chart dashboard with a SIGNAL readiness bar
- Pop-up, mobile push, and Telegram alerts
- Quick User Guide (PDF) and setup support

## Free option
- No free plan. You can evaluate the EA before going live by running it on a MetaTrader 5 demo account and in the MT5 Strategy Tester (backtest).

## Notes
- Not a subscription. Not martingale. Not grid.
- Trading is high-risk; test on demo and run a multi-month backtest before trading live.

## Contact
- support@goldscalpers.com
