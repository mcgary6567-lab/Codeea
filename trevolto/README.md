...
webhook alerts** — driven by the `Trevolto_AI_Crypto_Bot.pine` indicator in
...
| **Built-in strategy** | Runs the bot's own port of the Trevolto indicator on exchange candles — auto-trades with **no TradingView account** (Strategy tab) |
...
The built `Trevolto.exe` runs on **Windows 10 / 11 and Server 2016+ (64-bit)**
...
### Build a standalone `Trevolto.exe`
...
3. The result is **`dist\Trevolto.exe`** — copy it anywhere and run it.
...
- **Every push** to this branch → go to the repo's **Actions** tab → open the
  latest *Build Windows EXE* run → download the **`Trevolto-windows`**
  artifact (a zip containing `Trevolto.exe`).
...
## Auto-trading from TradingView (Trevolto indicator)

The flow:

```
Trevolto alert  →  TradingView webhook  →  this bot's receiver  →  exchange order
```
...
2. Keep the **Strategy filter** at **`Trevolto`** (the default). The bot then
   acts **only** on alerts whose `comment`/strategy tag matches the indicator's
   **Strategy tag** (default `Trevolto`) and ignores everything else. Leave the
...
4. Add `Trevolto_AI_Crypto_Bot.pine` to a **crypto** chart, then create an alert:
...
5. The indicator sends payloads like (Entry + TP1 + TP2, **no stop-loss**):
   ...
   ```json
   {"action":"buy","symbol":"BTCUSDT","passphrase":"","entry":67500,"tp1":68200,"tp2":69500,"comment":"Trevolto"}
   ```
...
The **Strategy** tab runs the bot's own port of the `Trevolto_AI_Crypto_Bot.pine`
...
curl -X POST http://127.0.0.1:8723/ -H "Content-Type: application/json" ^
  -d "{\"action\":\"buy\",\"symbol\":\"BTC/USDT\",\"entry\":67500,\"tp1\":68200,\"tp2\":69500,\"comment\":\"Trevolto\"}"
```
...
*"ignored (strategy … != Trevolto)"*, your **Strategy filter** doesn't match.
...
| `strategy.py` | Pure port of the Trevolto indicator (dip→green engine) |
