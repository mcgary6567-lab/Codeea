...
def licence_status(verify_url: str, token: str, timeout: float = 10.0) -> dict:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "TrevoltoBot"})
    try:
...
def start_trial(trial_url: str, email: str, machine: str,
    ...
    req = urllib.request.Request(trial_url, data=body, headers={"User-Agent": "TrevoltoBot"})
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            data = json.loads(resp.read().decode("utf-8"))
    except urllib.error.HTTPError as exc:
...
def request_pin_reset(reset_url: str, email: str, machine: str,
    ...
    req = urllib.request.Request(reset_url, data=body, headers={"User-Agent": "TrevoltoBot"})
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            data = json.loads(resp.read().decode("utf-8"))
    except urllib.error.HTTPError as exc:
