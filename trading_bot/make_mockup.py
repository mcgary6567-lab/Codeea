...
Preview of the Prometheus AI Crypto Bot main window — dark theme, branded
header with a mode badge + gear, setup strip, API panel with Chart / Backtest /
Analytics, open positions running, the 5-tab Trade Settings, and the trade log.
...
try:
    ...
    ab = AnnotationBbox(OffsetImage(logo, zoom=0.15), (0.5, 8.13), frameon=False, zorder=5)
    ax.add_artist(ab)
except Exception:
    pass
...
xx = 6.6
for label, on in [("Strategy ●", True), ("Webhook ○", False), ("Cloud ●", True), ("Paused ○", False)]:
...
# ===== LEFT: open positions =====
panel(0.35, 0.4, 6.0, 3.35, title="Open Positions")
